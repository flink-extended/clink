...
const Feature* MultiHot::Evaluate(Context* context) {
  ...
  auto child_ptr = GetFeatureValues<std::string>(*child);
  ...
  if (truncate_num_ > 0 && truncate_num_ <= bin_size_ + 1) {
    ...
    for (int i = 0; i <= bin_size_; ++i) {
      ...
      // 插入值为1的索引
      IVRecordEntry* iv_record_entry = context->CreateMessageT<IVRecordEntry>();
      if (index_vec.find(i) != index_vec.end()) {
        iv_record_entry->set_index(i);
        iv_record_entry->mutable_value()->mutable_int_list()->add_value(1);
        GetFeatureValues<IVRecordEntry>(output)->Add()->Swap(iv_record_entry);
        ++act_size;
    ...
    AppendFeatureValues(index_vec, output);
  }
  return output;
}
