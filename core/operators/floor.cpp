...
Floor::Floor() : UnaryOperator() {}

Floor::Floor(const std::string& feature_name) : UnaryOperator(feature_name) {
  children_.emplace_back(std::make_shared<Variable>(feature_name));
  init_status_ = true;
}

Floor::Floor(const Floor& node) : UnaryOperator(node) {}
int Floor::Evaluate(const FeatureMap& feature_map,
  ...
  GetFeatureValues<int64_t>(feature.get())->Add(floor(d_res));
  return STATUS_OK;
}

std::shared_ptr<BaseOperator> Floor::Clone() const {
