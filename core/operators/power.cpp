...
Power::Power(const std::string& first_feature,
...
int Power::Evaluate(const FeatureMap& feature_map,
                    std::shared_ptr<Feature>& feature) {
  feature = std::make_shared<Feature>();
  double left = 0, right = 0;
  GetTreeValue(feature_map, left, right);
  feature->mutable_double_list()->add_value(pow(left, right));
  return STATUS_OK;
}

std::shared_ptr<BaseOperator> Power::Clone() const {
